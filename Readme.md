...
The starting values of the **magnetic fluxes**, **dimensions of the slots** and other parametars which must be declared by the designer are generated by the program. A matrix is generated with the all posible combinations of this parametars. After this parametars are generated, the program start calculating the other parametars of the motor. When all parametars are calculated, the program check if the calculated parametars satisfy the conditions which are given by the designer. If are satisfied, the program writes the parametar values in *Parametars.csv*, found in *designed_motor_parametars* folder. Else the program pick another combination from the matrix and start the calcualion from begining.
